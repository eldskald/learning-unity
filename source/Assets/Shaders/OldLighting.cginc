...
float4x4 GetVertexLightColor (float3 worldPos, float3 normal) {
...
Surface GetSurface(Interpolators i) {
    ...
    s.albedo = _Color.rgb * tex2D(_MainTex, uv).rgb * tex2D(_DiffuseGradient, uv).rgb;
...
half4 frag (Interpolators i) : SV_TARGET {
    ...
    // Final color influenced by scene lights.
    half4 col;
    ...
    // rendering tutorial by Catlike Coding.
        ...
        #if defined(VERTEXLIGHT_ON)
            col.rgb += i.vertexLightColor._m00_m01_m02 * s.albedo;
        #endif
